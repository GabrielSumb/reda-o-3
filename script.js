...
function updateCountdown() {
  // 19 dezembro 2025 12:20 Brasília é UTC-3 → convertendo para UTC
...
messageForm.addEventListener('submit', e => {
  ...
  messages.push({ name, message: msg, date: new Date() });
  renderMessages();
  messageForm.reset();
});
...
// REDAÇÕES (14 redações — substitua os textos abaixo pelos que você tem)
const essays = [
...
function buildEssaysAccordion() {
  ...
  essays.forEach((essay, i) => {
    ...
    readBtn.addEventListener('click', () => speakText(essay.texto));
